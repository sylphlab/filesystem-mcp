 ...
 */
export async function cleanupTemporaryFilesystem(
  dirPath: string,
): Promise<void> {
  if (!dirPath) {
  ...
  try {
    ...
    if (!path.basename(dirPath).startsWith('jest-temp-')) {
      console.error(
        `Refusing to delete directory not matching 'jest-temp-*' pattern: ${dirPath}`,
      );
...
export async function createTemporaryFilesystem(
  ...
  try {
    ...
    console.error(
      `Failed to create temporary filesystem at ${tempDirPath}:`,
      error,
    );
