...
const { moveItemsToolDefinition } = await import(
  '../../src/handlers/moveItems.js'
);
...
describe('handleMoveItems Integration Tests', () => {
  ...
  it('should move a file to a new location (rename)', async () => {
    ...
    // Verify move
    await expect(
      fsPromises.access(path.join(tempRootDir, 'fileToMove.txt')),
    ).rejects.toThrow(/ENOENT/);
    const content = await fsPromises.readFile(
      path.join(tempRootDir, 'movedFile.txt'),
      'utf-8',
    );
  ...
  it('should move a file into an existing directory', async () => {
    const request = {
      operations: [
        {
          source: 'fileToMove.txt',
          destination: 'existingTargetDir/fileToMove.txt',
        },
      ],
    };
    const rawResult = await moveItemsToolDefinition.handler(request);
    ...
    const content = await fsPromises.readFile(
      path.join(tempRootDir, 'existingTargetDir/fileToMove.txt'),
      'utf-8',
    );
    expect(content).toBe('Move me!');
  });
  ...
  it('should move a directory to a new location (rename)', async () => {
    ...
    expect(result[0]).toEqual({
      source: 'dirToMove',
      destination: 'movedDir',
      success: true,
    });

    // Verify move
    await expect(
      fsPromises.access(path.join(tempRootDir, 'dirToMove')),
    ).rejects.toThrow(/ENOENT/);
    const stats = await fsPromises.stat(path.join(tempRootDir, 'movedDir'));
    expect(stats.isDirectory()).toBe(true);
    const content = await fsPromises.readFile(
      path.join(tempRootDir, 'movedDir/nestedFile.txt'),
      'utf-8',
    );
  ...
  it('should move a directory into an existing directory', async () => {
    const request = {
      operations: [
        { source: 'dirToMove', destination: 'existingTargetDir/dirToMove' },
      ],
    };
    const rawResult = await moveItemsToolDefinition.handler(request);
    const result = JSON.parse(rawResult.content[0].text);
    ...
    expect(result[0]).toEqual({
      source: 'dirToMove',
      destination: 'existingTargetDir/dirToMove',
      success: true,
    });
    ...
    const stats = await fsPromises.stat(
      path.join(tempRootDir, 'existingTargetDir/dirToMove'),
    );
    expect(stats.isDirectory()).toBe(true);
    const content = await fsPromises.readFile(
      path.join(tempRootDir, 'existingTargetDir/dirToMove/nestedFile.txt'),
      'utf-8',
    );
    expect(content).toBe('I am nested.');
  });
  ...
  it('should return error if destination parent directory does not exist', async () => {
    const request = {
      operations: [
        { source: 'fileToMove.txt', destination: 'nonexistentDir/moved.txt' },
      ],
    };
    const rawResult = await moveItemsToolDefinition.handler(request);
    const result = JSON.parse(rawResult.content[0].text);
    ...
    await expect(
      fsPromises.access(path.join(tempRootDir, 'nonexistentDir/moved.txt')),
  ...
  it('should return error if destination is an existing file (and source is file/dir)', async () => {
    const request = {
      operations: [
        { source: 'fileToMove.txt', destination: 'anotherFile.txt' },
      ],
    };
    const rawResult = await moveItemsToolDefinition.handler(request);
    const result = JSON.parse(rawResult.content[0].text);
    ...
    const content = await fsPromises.readFile(
      path.join(tempRootDir, 'anotherFile.txt'),
      'utf-8',
    );
  ...
  it('should handle multiple operations with mixed results', async () => {
    ...
    await expect(
      fsPromises.access(path.join(tempRootDir, 'movedOkay.txt')),
    ).resolves.toBeUndefined();
    // Verify file involved in failed traversal wasn't moved
    await expect(
      fsPromises.access(path.join(tempRootDir, 'anotherFile.txt')),
    ).resolves.toBeUndefined();
  });
  ...
  it('should return error for absolute destination path (caught by mock resolvePath)', async () => {
    ...
    const request = {
      operations: [{ source: 'fileToMove.txt', destination: absoluteDest }],
    };
    const rawResult = await moveItemsToolDefinition.handler(request);
    const result = JSON.parse(rawResult.content[0].text);
    expect(result).toHaveLength(1);
  ...
  it('should reject requests with empty operations array based on Zod schema', async () => {
    ...
    await expect(moveItemsToolDefinition.handler(request)).rejects.toThrow(
      McpError,
    );
    await expect(moveItemsToolDefinition.handler(request)).rejects.toThrow(
  ...
  it('should handle unexpected errors during path resolution', async () => {
    ...
    await fsPromises.writeFile(
      path.join(tempRootDir, errorSourcePath),
      'content',
    );
    ...
    const request = {
      operations: [{ source: errorSourcePath, destination: validDestPath }],
    };
    const rawResult = await moveItemsToolDefinition.handler(request);
    const result = JSON.parse(rawResult.content[0].text);
    ...
    expect(errorResult.error).toMatch(
      /Failed to move item: Simulated generic resolve error on source/,
    );
  });
  ...
  it('should return error when attempting to move the project root', async () => {
    ...
    const request = {
      operations: [
        { source: rootPathIdentifier, destination: destinationPath },
      ],
    };
    const rawResult = await moveItemsToolDefinition.handler(request);
    const result = JSON.parse(rawResult.content[0].text);
    ...
    const errorResult = result.find(
      (r: any) => r.source === rootPathIdentifier,
    );
    expect(errorResult).toBeDefined();
    expect(errorResult.success).toBe(false);
