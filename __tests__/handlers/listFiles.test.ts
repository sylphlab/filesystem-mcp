...
describe('listFiles Handler (Integration)', () => {
  ...
  beforeEach(async () => {
    ...
    try {
      tempTestDir = await fsPromises.mkdtemp(
        path.join(process.cwd(), 'temp-test-listFiles-'),
      );
    } catch (err) {
    ...
    const actualStatsUtils = await vi.importActual<
      typeof import('../../src/utils/statsUtils.js')
    ...
    mockDependencies = {
      ...
      resolvePath: vi
        .fn()
        .mockImplementation((relativePath: string): string => {
  ...
  it('should list files non-recursively without stats', async () => {
    ...
    await fsPromises.writeFile(
      path.join(tempTestDir!, 'subdir', 'nested.txt'),
      'content2',
    );
    ...
    expect(resultData).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          path: `${testDirPathRelative}/fileA.ts`.replace(/\\/g, '/'),
  ...
  it('should throw McpError if path does not exist', async () => {
    ...
    await expect(
      ...
      message: expect.stringContaining(
        'Path not found: nonexistent-dir/nonexistent-file.txt',
      ),
    });
  });
  ...
  it('should handle errors during glob execution', async () => {
    ...
    await expect(
      ...
      message: expect.stringContaining(
        'Failed to list files using glob: Mocked glob error',
  ...
  it('should handle unexpected errors during initial stat', async () => {
    ...
    mockStat.mockImplementation(
      async (p: PathLike, opts: StatOptions | undefined) => {
        // Compare absolute paths now since resolvePath returns absolute
        const targetAbsolutePath =
          mockDependencies.resolvePath(testDirPathRelative);
    ...
    await expect(
      ...
      message: expect.stringContaining(
        'Failed to process path: Mocked initial stat error',
      ),
    });
  ...
  it('should handle stat errors gracefully when include_stats is true', async () => {
    ...
    expect(file2Result.stats.error).toMatch(
      /Could not get stats: Mocked stat error/,
  ...
  it('should throw McpError for invalid argument types (Zod validation)', async () => {
    ...
    await expect(
      ...
      message: expect.stringContaining(
        'recursive (Expected boolean, received string)',
      ), // Check Zod error message
    });
  });
