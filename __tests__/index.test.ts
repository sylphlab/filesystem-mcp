...
const mockHandlers = Object.values(allHandlers).reduce<Record<string, HandlerDefinition & { handler: ReturnType<typeof vi.fn> }>>((acc, handlerDef) => {
...
vi.mock('../src/handlers/index.js', () => ({ // Also update path here
    ...typedMockHandlers
}));
...
const mockProcessExit = vi.spyOn(process, 'exit').mockImplementation((() => { throw new Error('process.exit called'); }) as (code?: number | string | null | undefined) => never);
