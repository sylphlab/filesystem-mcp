...
const handleReplaceContentFunc = async (
  ...
  try {
    for (const relativePath of relativePaths) {
      ...
      let targetPath = ''; // Initialize for use in finally block
      let fileContent = '';
      let originalContent = '';
      ...
      try {
        ...
        for (const op of operations) {
          ...
          const searchRegex = useRegex
            ...
            : new RegExp(
                ...
                regexFlags,
              ); // Escape non-regex search string
        ...
        // Use conditional spread for error to satisfy exactOptionalPropertyTypes
    ...
    console.error(
      `[Filesystem MCP - replaceContent] Error during replace_content execution:`,
      error,
    );
    throw new McpError(
      ErrorCode.InternalError,
      `Failed during replace_content: ${error.message}`,
    );
  }

  // Sort results by original path order for predictability
  fileProcessingResults.sort(
    (a, b) =>
      relativePaths.indexOf(a.file ?? '') - relativePaths.indexOf(b.file ?? ''),
