...
import { promises as fsPromises, Stats, Dirent, StatOptions, PathLike } from "fs";
import path from "path";
import { z } from 'zod';
import { glob as globFn, Path as GlobPath, GlobOptions } from 'glob';
...
export interface ListFilesDependencies {
    ...
    PROJECT_ROOT: string;
    formatStats: (relativePath: string, absolutePath: string, stats: Stats) => ReturnType<typeof formatStatsUtil>;
...
export const handleListFilesFunc = async (deps: ListFilesDependencies, args: unknown) => {
  ...
  try {
    ...
    if (initialStats.isFile()) {
      const statsResult = deps.formatStats(relativeInputPath, targetAbsolutePath, initialStats);
      const outputJson = JSON.stringify(statsResult, null, 2);
      return { content: [{ type: "text", text: outputJson }] };
    }

    if (initialStats.isDirectory()) {
      let results: { path: string; stats?: ReturnType<typeof deps.formatStats> | { error: string } }[] = [];

      // Corrected logical AND: &&
      if (!recursive && !includeStats) {
          // Request Dirent objects, the return type should be Dirent[]
          const entries = await deps.readdir(targetAbsolutePath, { withFileTypes: true }) as Dirent[]; // Assert as Dirent[]
          for (const entry of entries) {
              ...
              try {
                  ...
                  if (entry.isDirectory()) {
                      isDirectory = true;
                  } else if (entry.isFile()) { // Check isFile() as well
                      isDirectory = false;
                  } else {
                  ...
                  console.warn(`[Filesystem MCP - listFiles] Could not determine type for item ${itemRelativePath} during readdir: ${statError.message}`);
              }
          ...
          try {
              ...
              for (const entry of pathsFromGlob) {
                  ...
                  let statsResult: ReturnType<typeof deps.formatStats> | { error: string } | undefined = undefined;
                  const pathRelativeGlob = entry as string;
                  const relativeToRoot = deps.path.join(relativeInputPath, pathRelativeGlob);
                  const absolutePath = deps.path.resolve(targetAbsolutePath, pathRelativeGlob);
                  ...
                  try {
                      ...
                      if (includeStats) {
                          statsResult = deps.formatStats(relativeToRoot, absolutePath, entryStats);
                      }
                  } catch (statError: any) {
                      console.warn(`[Filesystem MCP - listFiles] Could not get stats for ${relativeToRoot}: ${statError.message}`);
                      if (includeStats) {
    ...
    if (error.code === 'ENOENT') throw new McpError(ErrorCode.InvalidRequest, `Path not found: ${relativeInputPath}`, { cause: error });
    if (error instanceof McpError) throw error;
    console.error(`[Filesystem MCP] Error in listFiles for ${targetAbsolutePath}:`, error);
    throw new McpError(ErrorCode.InternalError, `Failed to process path: ${error.message}`, { cause: error });
  }
};
