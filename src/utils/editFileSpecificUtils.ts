...
import { getIndentation, applyIndentation, linesMatch } from './stringUtils.js';
...
export function calculateIndentForReplacement(
  ...
  const contentUpToMatch = currentContent.substring(0, startIndex);
  const linesUpToMatch = contentUpToMatch.split('\n');
  // Use imported util
  return getIndentation(linesUpToMatch[linesUpToMatch.length - 1]);
}
...
export function findNthPlainTextMatch(
  ...
  const {
    ...
    occurrence,
  } = params; // Destructure from the params object
  let occurrencesFound = 0;
  ...
  return null; // Match not found for the specified occurrence
}
